...
CREATE TABLE IF NOT EXISTS files (
    ...
    processing_started_at timestamptz DEFAULT now(),  -- when the current extraction/ingest run began
    last_heartbeat_at timestamptz,           -- liveness stamp from the worker (every 45s); the sweeper reclaims rows silent > 3 min
